...
<script>
import { formatPhoneNumber, handleInput as handleInputMask, handleKeydown as handleKeydownMask } from '@/utils/phoneMask';
...
export default {
  ...
  watch: {
    value: {
      immediate: true,
      handler(newValue) {
        this.formattedValue = this.mask ? formatPhoneNumber(newValue) : newValue;
      },
    },
  },
  methods: {
    handleInput(event) {
      if (this.mask) {
        handleInputMask(event, this.mask, this, this.$emit);
      } else {
    ...
    handleKeydown(event) {
      handleKeydownMask(event, this.mask);
    },
